...
Scene transition: ${scene.continuity?.enters_from || 'continues from previous scene energy'} → this scene. Exits to: ${scene.continuity?.exits_to || 'next scene'}. Ensure the emotional transition between scenes engages the viewer completely and convinces them of the script's absolute truth.`;
  ...
  const energyDirection = roleData.energy + "\n\nELITE HOLLYWOOD-CALIBER ACTING REQUIRED: Optimise each scene to profound cinematic perfection having in Mind The Full Video we're directing. The Acting Performance must be World class! Inject hyper-detailed micro-expressions, subtle jaw tension, and intense pupil focus. Optimise The Emotional Arc to perfection, and Ensure the Directing is Optimised to Retain attention as much as possible and to be as persuasive as possible. Engineer Each Scene to magnetic perfection please. The Realism must be impeccable. You must stick strictly to the target frames provided.";
  const directorNote    = roleData.directorNote + "\n\nELITE EMOTIONAL DEPTH: Optimise this scene to absolute cinematic perfection. The Facial Expressions must convey profound depth (e.g., asymmetrical brow raises, locked-in gaze, calculating micro-shifts). Ensure the Directing is Optimised to Retain attention as much as possible and to be as persuasive as possible. Engineer Each Scene to perfection please. The Realism must be impeccable. You must stick strictly to the target frames provided.";
...
The face: before the first word, the face already holds the full weight of what this scene is about to say. What is leaking through composure — describe hyper-detailed micro-expressions of ${emotionalCore} (e.g., subtle jaw tension, an asymmetrical brow raise, calculating micro-shifts, intense pupil focus). The eyes: ${referenceAnalysis.character?.acting_style?.eye_behavior || 'direct, warm, absolutely held — the ease of someone accustomed to consequential conversations'}. The facial expressions must convey profound emotional depth and elite persuasion. The body and hands: ${physicalSig} — describe exactly what triggers movement and where the body arrives. Every gesture is discovered, never performed. Every frame is a still worth pausing on — not because it is theatrical, but because this person is this present and this genuinely engaged.]
...
[Action & Performance]
High-converting elite VSL presenter performance optimized for maximum psychological retention and absolute persuasive power. The performance radiates sovereign certainty and peer-level respect for wealthy investors. Elite Hollywood-caliber facial acting: profound emotional depth conveyed through hyper-detailed micro-expressions (e.g., subtle jaw clenching, calculating asymmetrical brow raises, and intense pupil focus). Confident, piercing expression paired with a single precise, authoritative hand gesture to emphasize the core point. Calculated conversational pacing utilizing intentional micro-pauses for pattern interruption and tension building. Unwavering direct-to-lens eye contact. The actor is utterly convinced of their own script.
